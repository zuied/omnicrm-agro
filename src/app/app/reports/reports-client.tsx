...
import { STAGE_BAR_COLORS, STAGE_ORDER } from "@/lib/stages";
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="space-y-4">
          {STAGE_ORDER.map((stage) => {
            ...
            const pct = totalDeals ? Math.round((s.cnt / totalDeals) * 100) : 0;
            return (
              <div key={stage}>
                ...
                <div className="h-4 w-full overflow-hidden rounded-lg bg-slate-100">
                  <div className={`h-full rounded-lg ${STAGE_BAR_COLORS[stage] ?? "bg-corporate"}`} style={{ width: `${Math.max(pct, s.cnt > 0 ? 4 : 0)}%` }} />
                </div>
              </div>
            );
          })}
        </div>
        <div className="mt-4 text-[11px] text-slate-400">{totalDeals} deal tercatat di seluruh tahap (termasuk Closed Won/Lost).</div>
      </div>
